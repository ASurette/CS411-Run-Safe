function launchRequest() {
    ...
    $(xml).find("edge").each(function(){
        ...
        var val=$(this).find('wkt').text();wkt.push(val.toString());
    });

    var hold=[];for(var i=0;i<wkt.length;i++) {
        var str=wkt[i];var a=str.indexOf("(");
        var z=str.indexOf(")");
        str=str.substring(a+1,z);
        var points=[];
        var points=str.split(",");
        var leg=[];for(var j=0;j<points.length;j++) {
            var LL=[];
            LL=points[j].split(" ");
            leg[j]=new LatLng(parseFloat(LL[1]),parseFloat(LL[0]));}
    hold.push(leg);
    }
    var steps=[];for(var i=0;i<wkt.length;i++) {
        var newStep=new step(instruction[i],dist[i],time[i],acc_dist[i],acc_time[i],hold[i]);steps.push(newStep);
    }
...
function examineRLroutes() {
    ...
    for(i=0;i<RLroutes[0].steps.length;i++) {
        ...
        for(var j=0;j<RLroutes[0].steps[i].leg.length;j++)
            document.write("leg "+j+"  "+RLroutes[0].steps[i].leg[j].lat+","+RLroutes[0].steps[i].leg[j].lng+"  ");
        document.write("<p>");}
}
function getRlWaypoints(nPoints) {
    ...
    TotalLength=0;CumDist.push(TotalLength);for(var i=1;i<polyline.length;i++) {
    ...
    for(var i=0;i<polyline.length;i++) {
        if(CumDist[i]>=point*chunk) {
            rlPoints.push(polyline[i]);point++;}
    }
